import re
from commons import FailToAnswerException
import random
...
class Eliza(object):
    ...
    def __init__(self):
        self.patterns = {re.compile(regex, re.IGNORECASE): answers for regex, answers in self._regexes.items()}
    ...
    def respond(self, query):
        for regex, answers in self.patterns.items():
            ...
            if match:
                ...
                while pos > -1:
                    ...
                    resp = resp[:pos] + self.translate(match.group(num), self._gReflections) + resp[pos + 2:]
                    pos = resp.find('%')
                # fix munged punctuation at the end
                if resp[-2:] == '?.': resp = resp[:-2] + '.'
                if resp[-2:] == '??': resp = resp[:-2] + '?'
                return resp.replace("{0}", query)
